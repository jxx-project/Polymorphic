			...
			mapType + " C++11 standard interface",
			{
				...
				TestCase(
					...
					[] {
						...
						TestCase::assert(
							std::is_same<
								typename MapType<Key, T, Compare, Allocator>::const_reference,
				...
				TestCase(
					...
					[] {
						...
						auto result = testee.rbegin();
						TestCase::assert(
							std::is_same<typename MapType<Key, T, Compare, Allocator>::const_reverse_iterator, decltype(result)>::
								value,
							"type");
						TestCase::assert(*result == ValueType(), "result");
					}),
				...
				TestCase(
					...
					[] {
						...
						if (!isMultimap) {
							TestCase::assert(
								std::is_same<
									std::pair<typename MapType<Key, T, Compare, Allocator>::iterator, bool>,
...
public:
	void operator()()
	{
		...
		auto& result = testee.at(Key());
		TestCase::assert(
			std::is_same<const typename MapType<Key, T, Compare, Allocator>::mapped_type&, decltype(result)>::value, "type");
		TestCase::assert(result == T(), "result");
	}
};
