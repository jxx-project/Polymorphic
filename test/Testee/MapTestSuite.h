...
template <
template <typename...> class MapType, typename Key, typename T, bool isMultimap, typename Compare, typename Allocator,
...
template <
template <typename...> class MapType, typename Key, typename T, bool isMultimap = false,
typename Compare = std::less<Key>, typename Allocator = std::allocator<std::pair<const Key, T>>>
...
public:
	...
	MapTestSuite(const std::string& mapType)
		: TestSuite(
		  mapType + " C++11 standard interface",
		  {
		  ...
		  [] {
			  TestCase::assert(
			  std::is_same<typename MapType<Key, T, Compare, Allocator>::key_type, Key>::value, "key_type");
			  TestCase::assert(
			  std::is_same<typename MapType<Key, T, Compare, Allocator>::mapped_type, T>::value, "mapped_type");
			  TestCase::assert(
			  std::is_same<typename MapType<Key, T, Compare, Allocator>::value_type, std::pair<const Key, T>>::value,
			  "value_type");
			  TestCase::assert(
			  std::is_class<typename MapType<Key, T, Compare, Allocator>::allocator_type>::value, "allocator_type");
			  TestCase::assert(
			  std::is_same<typename MapType<Key, T, Compare, Allocator>::reference, std::pair<const Key, T>&>::value,
			  "reference");
			  TestCase::assert(
			  std::is_same<
			  typename MapType<Key, T, Compare, Allocator>::const_reference, const std::pair<const Key, T>&>::value,
			  "const_reference");
		  }),
		  ...
		  [] {
			  ...
			  TestCase::assert(
			  std::is_same<typename MapType<Key, T, Compare, Allocator>::iterator, decltype(result)>::value, "type");
		  ...
		  [] {
			  ...
			  auto result = testee.begin();
			  TestCase::assert(
			  std::is_same<typename MapType<Key, T, Compare, Allocator>::const_iterator, decltype(result)>::value,
			  "type");
			  TestCase::assert(*result == ValueType(), "result");
		  }),
		  ...
		  [] {
			  ...
			  TestCase::assert(
			  std::is_same<typename MapType<Key, T, Compare, Allocator>::reverse_iterator, decltype(result)>::value,
			  "type");
			  TestCase::assert(*result == ValueType(), "result");
		  }),
		  ...
		  [] {
			  ...
			  TestCase::assert(
			  std::is_same<
			  typename MapType<Key, T, Compare, Allocator>::const_reverse_iterator, decltype(result)>::value,
			  "type");
			  TestCase::assert(*result == ValueType(), "result");
		  }),
		  ...
		  [] {
			  ...
			  TestCase::assert(
			  std::is_same<typename MapType<Key, T, Compare, Allocator>::size_type, decltype(result)>::value, "type");
		  ...
		  [] {
			  ...
			  if (!isMultimap) {
				  TestCase::assert(
				  std::is_same<
				  std::pair<typename MapType<Key, T, Compare, Allocator>::iterator, bool>, decltype(result)>::value,
				  "type");
			  } else {
				  TestCase::assert(
				  std::is_same<typename MapType<Key, T, Compare, Allocator>::iterator, decltype(result)>::value,
				  "type");
			  }
		  ...
		  [] {
			  ...
			  TestCase::assert(
			  std::is_same<typename MapType<Key, T, Compare, Allocator>::key_compare, decltype(result)>::value);
		  }),
		  ...
		  [] {
			  ...
			  TestCase::assert(
			  std::is_same<typename MapType<Key, T, Compare, Allocator>::value_compare, decltype(result)>::value);
		  }),
		  ...
		  [] {
			  ...
			  TestCase::assert(std::is_same<
							   std::pair<
							   typename MapType<Key, T, Compare, Allocator>::iterator,
							   typename MapType<Key, T, Compare, Allocator>::iterator>,
		  ...
		  [] {
			  ...
			  TestCase::assert(std::is_same<
							   std::pair<
							   typename MapType<Key, T, Compare, Allocator>::const_iterator,
							   typename MapType<Key, T, Compare, Allocator>::const_iterator>,
							   decltype(result)>::value);
		  }),
		  ...
		  [] {
			  ...
			  TestCase::assert(
			  std::is_same<typename MapType<Key, T, Compare, Allocator>::allocator_type, decltype(result)>::value);
		  }),
...
template <
template <typename...> class MapType, typename Key, typename T, bool isMultimap, typename Compare, typename Allocator>
const typename MapTestSuite<MapType, Key, T, isMultimap, Compare, Allocator>::InitializerListType
MapTestSuite<MapType, Key, T, isMultimap, Compare, Allocator>::INITIALIZER_LIST = {std::make_pair(Key(), T())};

template <
template <typename...> class MapType, typename Key, typename T, bool isMultimap, typename Compare, typename Allocator>
const typename MapTestSuite<MapType, Key, T, isMultimap, Compare, Allocator>::ArrayType
MapTestSuite<MapType, Key, T, isMultimap, Compare, Allocator>::ARRAY = {std::make_pair(Key(), T())};
...
public:
	...
	{
		MapType<Key, T, Compare, Allocator> testee(
		MapTestSuite<MapType, Key, T, isMultimap, Compare, Allocator>::INITIALIZER_LIST);
		auto& result = testee[Key()];
		TestCase::assert(
		std::is_same<typename MapType<Key, T, Compare, Allocator>::mapped_type&, decltype(result)>::value, "type");
...
template <typename...> class MapType, typename Key, typename T, bool isMultimap, typename Compare, typename Allocator>
class TestSubscriptOperatorMove<
MapType, Key, T, isMultimap, Compare, Allocator, typename std::enable_if<!isMultimap>::type>
{
public:
	void operator()() const
	{
...
public:
	void operator()()
	{
		...
		auto& result = testee.at(Key());
		TestCase::assert(
		std::is_same<const typename MapType<Key, T, Compare, Allocator>::mapped_type&, decltype(result)>::value,
		"type");
		TestCase::assert(result == T(), "result");
	}
};
