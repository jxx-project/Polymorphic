			...
			vectorType + " C++11 standard interface",
			{
				...
				TestCase(
					...
					[] {
						...
						if (!std::is_same<bool, T>::value) {
							...
							TestCase::assert(
								std::is_same<typename VectorType<T, Allocator>::const_reference, const T&>::value,
								"const_reference");
						} else {
				...
				TestCase(
					...
					[] {
						...
						auto result = testee.rbegin();
						TestCase::assert(
							std::is_same<typename VectorType<T, Allocator>::const_reverse_iterator, decltype(result)>::value,
							"type");
						TestCase::assert(*result == T(), "result");
					}),
