...
template <template <typename...> class VectorType, typename T, typename Allocator, typename Enable = void> class TestAt;
...
public:
	...
	VectorTestSuite(const std::string& vectorType)
		: TestSuite(
		  ...
		  [] {
			  ...
			  TestCase::assert(
			  std::is_class<typename VectorType<T, Allocator>::allocator_type>::value, "allocator_type");
			  if (!std::is_same<bool, T>::value) {
				  TestCase::assert(std::is_same<typename VectorType<T, Allocator>::reference, T&>::value, "reference");
				  TestCase::assert(
				  std::is_same<typename VectorType<T, Allocator>::const_reference, const T&>::value, "const_reference");
			  } else {
				  TestCase::assert(std::is_class<typename VectorType<T, Allocator>::reference>::value, "reference");
				  TestCase::assert(
				  std::is_same<typename VectorType<T, Allocator>::const_reference, T>::value, "const_reference");
			  }
		  }),
		  ...
		  [] {
			  ...
			  TestCase::assert(
			  std::is_same<typename VectorType<T, Allocator>::iterator, decltype(result)>::value, "type");
		  ...
		  [] {
			  ...
			  auto result = testee.begin();
			  TestCase::assert(
			  std::is_same<typename VectorType<T, Allocator>::const_iterator, decltype(result)>::value, "type");
			  TestCase::assert(*result == T(), "result");
		  }),
		  ...
		  [] {
			  ...
			  TestCase::assert(
			  std::is_same<typename VectorType<T, Allocator>::reverse_iterator, decltype(result)>::value, "type");
			  TestCase::assert(*result == T(), "result");
		  }),
		  ...
		  [] {
			  ...
			  auto result = testee.rbegin();
			  TestCase::assert(
			  std::is_same<typename VectorType<T, Allocator>::const_reverse_iterator, decltype(result)>::value, "type");
			  TestCase::assert(*result == T(), "result");
		  }),
		  ...
		  [] {
			  ...
			  TestCase::assert(
			  std::is_same<typename VectorType<T, Allocator>::size_type, decltype(result)>::value, "type");
		  ...
		  [] {
			  ...
			  TestCase::assert(
			  std::is_same<typename VectorType<T, Allocator>::allocator_type, decltype(result)>::value);
		  }),
...
const typename VectorTestSuite<VectorType, T, Allocator>::InitializerListType
VectorTestSuite<VectorType, T, Allocator>::INITIALIZER_LIST = {T()};

template <template <typename...> class VectorType, typename T, typename Allocator>
const typename VectorTestSuite<VectorType, T, Allocator>::ArrayType VectorTestSuite<VectorType, T, Allocator>::ARRAY = {
T()};
...
public:
	void operator()()
	{
		...
		auto& result = testee[0];
		TestCase::assert(
		std::is_same<typename VectorType<T, Allocator>::const_reference, decltype(result)>::value, "type");
		TestCase::assert(result == T(), "result");
	}
};
...
public:
	void operator()()
	{
		...
		auto result = testee.data();
		TestCase::assert(
		std::is_same<typename VectorType<T, Allocator>::const_pointer, decltype(result)>::value, "const_pointer");
		TestCase::assert(*result == T(), "result");
	}
};
