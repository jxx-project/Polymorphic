...
template<
	template<typename...> class UnorderedMapType,
	typename Key,
	typename T,
	bool isMultimap,
	typename Hash,
			...
			unorderedMapType + " C++11 standard interface",
			{
					...
					[] {
						...
						TestCase::assert(
							std::is_same<
								...
								typename UnorderedMapType<Key, T, Hash, Predicate, Allocator>::allocator_type::const_reference>::
								value,
							"const_reference");
					}),
				...
				TestCase(
					...
					[] {
						...
						TestCase::assert(
							std::is_same<
								typename UnorderedMapType<Key, T, Hash, Predicate, Allocator>::size_type,
								decltype(result)>::value,
							"type");
				...
				TestCase(
					...
					[] {
						...
						TestCase::assert(
							std::is_same<typename UnorderedMapType<Key, T, Hash, Predicate, Allocator>::hasher, decltype(result)>::
								value);
					}),
				...
				TestCase(
					...
					[] {
						...
						TestCase::assert(std::is_same<
										 typename UnorderedMapType<Key, T, Hash, Predicate, Allocator>::key_equal,
										 decltype(result)>::value);
					}),
				...
				TestCase(
					...
					[] {
						...
						TestCase::assert(std::is_same<
										 typename UnorderedMapType<Key, T, Hash, Predicate, Allocator>::allocator_type,
										 decltype(result)>::value);
					}),
...
template<
	...
	typename Predicate,
	typename Allocator>
const typename UnorderedMapTestSuite<UnorderedMapType, Key, T, isMultimap, Hash, Predicate, Allocator>::InitializerListType
...
const typename UnorderedMapTestSuite<UnorderedMapType, Key, T, isMultimap, Hash, Predicate, Allocator>::ArrayType
	UnorderedMapTestSuite<UnorderedMapType, Key, T, isMultimap, Hash, Predicate, Allocator>::ARRAY = {std::make_pair(Key(), T())};
...
class TestSubscriptOperator<
	...
	Predicate,
	Allocator,
	typename std::enable_if<!isMultimap>::type>
{
public:
	void operator()()
	{
		...
		TestCase::assert(
			std::is_same<typename UnorderedMapType<Key, T, Hash, Predicate, Allocator>::mapped_type&, decltype(result)>::value,
			"type");
...
public:
	void operator()()
	{
		...
		auto& result = testee.at(Key());
		TestCase::assert(
			std::is_same<const typename UnorderedMapType<Key, T, Hash, Predicate, Allocator>::mapped_type&, decltype(result)>::
				value,
			"type");
		TestCase::assert(result == T(), "result");
	}
