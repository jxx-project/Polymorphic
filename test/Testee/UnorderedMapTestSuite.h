...
template <
template <typename...> class UnorderedMapType, typename Key, typename T, bool isMultimap, typename Hash,
typename Predicate, typename Allocator, typename Enable = void>
...
template <
template <typename...> class UnorderedMapType, typename Key, typename T, bool isMultimap = false,
typename Hash = std::hash<Key>, typename Predicate = std::equal_to<Key>,
...
public:
	...
	UnorderedMapTestSuite(const std::string& unorderedMapType)
		: TestSuite(
		  ...
		  [] {
			  TestCase::assert(
			  std::is_same<typename UnorderedMapType<Key, T, Hash, Predicate, Allocator>::key_type, Key>::value,
			  "key_type");
			  TestCase::assert(
			  std::is_same<typename UnorderedMapType<Key, T, Hash, Predicate, Allocator>::mapped_type, T>::value,
			  "mapped_type");
			  TestCase::assert(
			  std::is_same<
			  typename UnorderedMapType<Key, T, Hash, Predicate, Allocator>::value_type,
			  std::pair<const Key, T>>::value,
			  "value_type");
			  TestCase::assert(
			  std::is_class<typename UnorderedMapType<Key, T, Hash, Predicate, Allocator>::hasher>::value, "hasher");
			  TestCase::assert(
			  std::is_class<typename UnorderedMapType<Key, T, Hash, Predicate, Allocator>::key_equal>::value,
			  "key_equal");
			  TestCase::assert(
			  std::is_class<typename UnorderedMapType<Key, T, Hash, Predicate, Allocator>::allocator_type>::value,
			  "allocator_type");
			  TestCase::assert(
			  std::is_same<
			  typename UnorderedMapType<Key, T, Hash, Predicate, Allocator>::reference,
			  typename UnorderedMapType<Key, T, Hash, Predicate, Allocator>::allocator_type::reference>::value,
			  "reference");
			  TestCase::assert(
			  std::is_same<
			  typename UnorderedMapType<Key, T, Hash, Predicate, Allocator>::const_reference,
			  typename UnorderedMapType<Key, T, Hash, Predicate, Allocator>::allocator_type::const_reference>::value,
			  "const_reference");
		  }),
		  ...
		  [] {
			  ...
			  TestCase::assert(
			  std::is_same<UnorderedMapType<Key, T, Hash, Predicate, Allocator>&, decltype(result)>::value, "type");
		  ...
		  [] {
			  ...
			  TestCase::assert(
			  std::is_same<
			  typename UnorderedMapType<Key, T, Hash, Predicate, Allocator>::iterator, decltype(result)>::value,
			  "type");
		  ...
		  [] {
			  ...
			  TestCase::assert(
			  std::is_same<
			  typename UnorderedMapType<Key, T, Hash, Predicate, Allocator>::const_iterator, decltype(result)>::value,
			  "type");
			  TestCase::assert(*result == ValueType(), "result");
		  }),
		  ...
		  [] {
			  ...
			  TestCase::assert(
			  std::is_same<
			  typename UnorderedMapType<Key, T, Hash, Predicate, Allocator>::size_type, decltype(result)>::value,
			  "type");
		  ...
		  TestCase(
		  "operator[]", TestSubscriptOperator<UnorderedMapType, Key, T, isMultimap, Hash, Predicate, Allocator>()),

		  TestCase(
		  "operator[] move",
		  TestSubscriptOperatorMove<UnorderedMapType, Key, T, isMultimap, Hash, Predicate, Allocator>()),
		  ...
		  [] {
			  ...
			  if (!isMultimap) {
				  TestCase::assert(
				  std::is_same<
				  std::pair<typename UnorderedMapType<Key, T, Hash, Predicate, Allocator>::iterator, bool>,
		  ...
		  [] {
			  ...
			  TestCase::assert(std::is_same<
							   std::pair<
							   typename UnorderedMapType<Key, T, Hash, Predicate, Allocator>::iterator,
							   typename UnorderedMapType<Key, T, Hash, Predicate, Allocator>::iterator>,
							   decltype(result)>::value);
		  }),
		  ...
		  [] {
			  ...
			  TestCase::assert(std::is_same<
							   std::pair<
							   typename UnorderedMapType<Key, T, Hash, Predicate, Allocator>::const_iterator,
							   typename UnorderedMapType<Key, T, Hash, Predicate, Allocator>::const_iterator>,
							   decltype(result)>::value);
		  }),
		  ...
		  [] {
			  ...
			  TestCase::assert(
			  std::is_same<
			  typename UnorderedMapType<Key, T, Hash, Predicate, Allocator>::hasher, decltype(result)>::value);
		  }),
		  ...
		  [] {
			  ...
			  TestCase::assert(
			  std::is_same<
			  typename UnorderedMapType<Key, T, Hash, Predicate, Allocator>::key_equal, decltype(result)>::value);
		  }),
		  ...
		  [] {
			  ...
			  TestCase::assert(
			  std::is_same<
			  typename UnorderedMapType<Key, T, Hash, Predicate, Allocator>::allocator_type, decltype(result)>::value);
		  }),
...
template <
template <typename...> class UnorderedMapType, typename Key, typename T, bool isMultimap, typename Hash,
typename Predicate, typename Allocator>
const typename UnorderedMapTestSuite<
UnorderedMapType, Key, T, isMultimap, Hash, Predicate, Allocator>::InitializerListType
UnorderedMapTestSuite<UnorderedMapType, Key, T, isMultimap, Hash, Predicate, Allocator>::INITIALIZER_LIST = {
std::make_pair(Key(), T())};

template <
template <typename...> class UnorderedMapType, typename Key, typename T, bool isMultimap, typename Hash,
typename Predicate, typename Allocator>
const typename UnorderedMapTestSuite<UnorderedMapType, Key, T, isMultimap, Hash, Predicate, Allocator>::ArrayType
UnorderedMapTestSuite<UnorderedMapType, Key, T, isMultimap, Hash, Predicate, Allocator>::ARRAY = {
std::make_pair(Key(), T())};
...
typename Predicate, typename Allocator>
class TestSubscriptOperator<
UnorderedMapType, Key, T, isMultimap, Hash, Predicate, Allocator, typename std::enable_if<!isMultimap>::type>
{
public:
	void operator()()
	{
		...
		TestCase::assert(
		std::is_same<
		typename UnorderedMapType<Key, T, Hash, Predicate, Allocator>::mapped_type&, decltype(result)>::value,
		"type");
...
public:
	void operator()()
	{
		...
		TestCase::assert(
		std::is_same<
		const typename UnorderedMapType<Key, T, Hash, Predicate, Allocator>::mapped_type&, decltype(result)>::value,
		"type");
		TestCase::assert(result == T(), "result");
	}
};
